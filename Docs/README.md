...
**Executive Summary:**
The project proposes a scalable network of **ESP32 microcontrollers** to actively monitor the Radio Frequency (RF) environment. This replaces expensive, reactive troubleshooting with a proactive system that identifies "dead zones," channel congestion, and security threats in real-time.

**Key Objectives:**
* **Reduce OpEx & CapEx:** Replace professional-grade spectrum analyzers (costing thousands) with sub-$20 ESP32 units.
* **Enhance Security:** Detect unauthorized "Rogue Access Points" that bypass corporate firewalls.
* **Proactive Optimization:** Enable IT teams to fix congestion issues before users file complaints.

**Target Stakeholders:**
* **IT Administrators:** For diagnosing specific user complaints.
* **NetOps:** For capacity planning and identifying dead zones.
* **Cybersecurity:** For receiving alerts on unauthorized SSIDs.
...
| :--- | :--- | :--- |
| **IoT Device** | **ESP32 (C++)** | Scans 2.4GHz Wi-Fi channels for RSSI and SSID data. Publishes telemetry via **MQTTS**. |
...
| **Client App** | **React** | A local web dashboard for visualizing heatmaps and signal graphs. |
...
* **Hardware Savings:** Transitioning from dedicated spectrum analyzers to ESP32s significantly lowers the barrier to entry for building-wide monitoring.
